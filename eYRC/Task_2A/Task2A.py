...
def cart_pendulum_dynamics(y, m, M, L, g, u):
    ...
    dy = np.zeros(4)
    dy[0] = y[1]
    dy[1] = (u + m * L * sin_theta * y[3]**2 - m * g * cos_theta * sin_theta) / (M + m * sin_theta**2)
    dy[2] = y[3]
    dy[3] = (g * sin_theta - cos_theta * ((u + m * L * sin_theta * y[3]**2) / (M + m * sin_theta**2))) / (L * (4/3 - (m * cos_theta**2) / (M + m * sin_theta**2)))
    return dy
